...
$myfile = fopen("task-diff-user-gold-ins-".date('Y-m-d H-i-s').".txt","w") or die("Unable to open file!");
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($gold_row['gold'] != 0 || $gold_detailed_row['goldsum'] != '') {
        if ($gold_row['gold'] !== $gold_detailed_row['goldsum']) {
            ...
            $diff = $gold_row['gold'] - $gold_detailed_row['goldsum'];
            if ($diff > 0) {
            $errorstr = 'INSERT INTO `zq_gold_detailed`(`uid`, `relatedid`, `operation`, `coin`, `description`, `dateline`, `IP`) VALUE ('.$row['uid'].', 0, "RCV",'.$diff.', "管理员后台操作",'.time().', 0);'.PHP_EOL;
            fwrite($myfile,$errorstr);
            } else {
             $errorstr = 'INSERT INTO `zq_gold_detailed`(`uid`, `relatedid`, `operation`, `coin`, `description`, `dateline`, `IP`) VALUE ('.$row['uid'].', 0, "TFR",'.$diff.', "管理员后台操作",'.time().', 0);'.PHP_EOL;
            fwrite($myfile,$errorstr);
            }
	    } else {
